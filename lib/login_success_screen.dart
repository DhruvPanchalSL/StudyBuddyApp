...
class _LoginSuccessScreenState extends State<LoginSuccessScreen>
  ...
  void initState() {
    ...
    _title1Slide = Tween<Offset>(begin: const Offset(0, 0.4), end: Offset.zero)
        .animate(
          CurvedAnimation(
            parent: _textCtrl,
            curve: const Interval(0, 0.55, curve: Curves.easeOut),
    ...
    _title2Slide = Tween<Offset>(begin: const Offset(0, 0.5), end: Offset.zero)
        .animate(
          CurvedAnimation(
            parent: _textCtrl,
            curve: const Interval(0.35, 1.0, curve: Curves.easeOut),
  ...
  Future<void> _runSequence() async {
    ...
    // ✅ Pop back to root — StreamBuilder already has HomeScreen as the base.
    // popUntil(first) removes LoginSuccessScreen AND AuthScreen from stack.
                  ...
                  Positioned.fill(
                    child: CustomPaint(
                      painter: _OrbitPainter(
                        ...
                        center: center,
                        opacity: ((_circleCtrl.value - 0.5) * 2).clamp(
                          0.0,
                          1.0,
                        ),
                      ),
...
class _RipplePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (int i = 0; i < 4; i++) {
      ...
      if (t <= 0) continue;
      final paint = Paint()
        ..color = color.withOpacity((1 - t) * 0.18)
        ..style = PaintingStyle.stroke
        ..strokeWidth = 2;
      canvas.drawCircle(center, t * 220, paint);
    }
  }
...
class _ConfettiPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (final p in pieces) {
      ...
      final dx = center.dx + math.cos(p.angle) * p.speed * eased;
      final dy =
          center.dy + math.sin(p.angle) * p.speed * eased + 180 * eased * eased;
      canvas.save();
      canvas.translate(dx, dy);
      canvas.rotate(p.rotationSpeed * progress * math.pi);
      final paint = Paint()
        ..color = p.color.withOpacity((1 - progress * 0.85).clamp(0.0, 1.0));
      if (p.isRect) {
        canvas.drawRRect(
...
class _OrbitPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (int i = 0; i < 6; i++) {
      ...
      final dx = center.dx + math.cos(angle) * 80;
      final dy = center.dy + math.sin(angle) * 80;
      final phase = (progress * 4 + i / 6) % 1.0;
